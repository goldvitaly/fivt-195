...
class Vertex {
    typedef unsigned int TypeNameVer;
public:
    virtual void add_neighbour(const TypeNameVer& nameVer, const Weight& weight) = 0;
...
public:
    ...
    void add_edge(const TypeNameVer& outVer, const TypeNameVer& inVer, const Weight& weight)
    {
        check_exist(outVer, true);
        check_exist(inVer, true);
        graph[outVer]->add_neighbour(inVer, weight);
    }
    Weight weight(const TypeNameVer& outVer, const TypeNameVer& inVer) const
    {
        return graph[outVer]->weight(inVer);
    }
