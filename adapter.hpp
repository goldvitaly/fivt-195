...
{
public:
	GraphAdapter(Graph& gr, const std::vector<VType>& values);
...
template<typename VType>
GraphAdapter<VType>::GraphAdapter(Graph& gr, const std::vector<VType>& values) : g(gr)
{
...
template<typename VType>
unsigned GraphAdapter<VType>::add(std::unique_ptr<Node> node, const VType& name)
{
...
template<typename VType>
unsigned GraphAdapter<VType>::add(std::unique_ptr<Node> node, const VType& name, const std::vector<VType>& friends)
{
	unsigned v = g.add(std::move(node));
	index[name] = v;
...
template<typename VType>
template<typename NodeType>
unsigned GraphAdapter<VType>::add(const VType& name)
{
...
template<typename VType>
template<typename NodeType>
unsigned GraphAdapter<VType>::add(const VType& name, const std::vector<VType>& friends)
{
	return add(std::unique_ptr<Node>(new NodeType()), name, friends);
}

template<typename VType>
void GraphAdapter<VType>::connect(const VType& v1, const VType& v2)
{
	return g.connect(index[v1], index[v2]);
}

template<typename VType>
bool GraphAdapter<VType>::areConnected(const VType& v1, const VType& v2) const
{
	return g.areConnected(index.find(v1)->second, index.find(v2)->second);
}

template<typename VType>
const Node& GraphAdapter<VType>::getNode(const VType& v) const
{
