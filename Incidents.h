...
public:
    ...
    explicit Edge() {}
    explicit Edge(size_t _to, const TWeight& _weight = TWeight()): to(_to), weight(_weight) {}
...
void addNeighbour(const Edge<TWeight> & newEdge) override
    ...
    {
        auto it =
            std::find_if(adj.begin(), adj.end(), [findingVertex] (Edge<TWeight> curEdge)
    {
