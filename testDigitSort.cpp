...
void test(int n, F generator, std::string type, SortClass Sort) {
	...
	std::generate(arr.begin(), arr.end(), generator);
	start = clock();
	digit_sort(arr.begin(), arr.end(), Sort);
...
void testAll() {
	...
	test <std::pair<ULL,ULL>> (500000, genPair, "\"pair of long long\"", BigPairSort());
